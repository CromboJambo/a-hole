    ...
    fn capture_and_record_change(
        ...
        let diff_result = match DiffComputer::compute(old_content, new_content, &file_type) {
            ...
            Err(e) => {
                ...
                DiffComputer::create_default_summary();
        ...
        let summary_json = DiffComputer::serialize_summary(&diff_result.summary);
        ...
        let change_id = db.record_change(
            ...
            "updated", // MVP: only track updates for now
            diff_result.format.as_str(),
            &summary_json,
            "{}", // Empty metadata for v0.1
        )?;
