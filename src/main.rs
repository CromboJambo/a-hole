...
/// A-hole CLI - Local config observer for terminal developers
#[derive(Parser)]
#[command(name = "a-hole")]
...
impl Cli {
    fn run(self) -> Result<()> {
        // Initialize tracing
        fmt()
            .with_env_filter(EnvFilter::from_default_env().add_directive("a-hole=debug".parse()?))
            .init();

        debug!("CLI command parsed: {:?}", self.command);

        // Open database
        let db = match db::Database::new(self.db_path) {
            Ok(d) => d,
            Err(e) => {
    ...
    fn cmd_init(db: &db::Database, paths: Option<Vec<String>>) -> Result<()> {
    ...
    fn cmd_watch(db: &db::Database, watch_defaults: bool) -> Result<()> {
        info!("Starting watch command");

        let config = observer::WatcherConfig {
        ...
        match observer::ConfigWatcher::start(&db_arc, config) {
            Ok(_) => {
    ...
    fn cmd_log(db: &db::Database, limit: usize, tool: Option<&str>, json: bool) -> Result<()> {
        ...
        if json {
            ...
            let display_changes: Vec<cli::ChangeDisplay> = changes
                .iter()
                .map(|c| cli::ChangeDisplay {
                    id: c.id,
            ...
            for change in &changes[..limit.min(changes.len())] {
                ...
                if !summary.keys_changed.is_empty() {
                    let keys = &summary.keys_changed[..summary.keys_changed.len().min(3)];
                    println!("  Keys: {}", keys.join(", "));
                    if summary.keys_changed.len() > 3 {
                        println!("  ... and {} more", summary.keys_changed.len() - 3);
                    }
                }
    ...
    fn cmd_show(db: &db::Database, id: i64, json: bool) -> Result<()> {
    ...
    fn cmd_revert(db: &db::Database, id: i64, force: bool) -> Result<()> {
                    ...
                    Some(content) => {
                        ...
                        if let Some(file) = watched_file {
                            ...
                            match std::fs::write(&file.normalized_path, &content) {
                                ...
                                Err(e) => {
                                    error!("Failed to write reverted content: {}", e);
    ...
    fn cmd_list(db: &db::Database, verbose: bool) -> Result<()> {
